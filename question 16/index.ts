...
let guest: string[] = ["Kareem", "Sumair", "Dawood"];

// Function to shrink the guest list
function shrinkGuestsList(maxsGuests: number) {
    if (maxsGuests >= guest.length) {
        console.log("No guests need to be removed. You can invite all guests.");
    } else {
        const guestsToRemove = guest.length - maxsGuests;
        guest.splice(-guestsToRemove);
        console.log(`Guest list shrunk to ${maxsGuests} guests.`);
